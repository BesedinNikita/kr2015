...
public class GameBoard extends JPanel{
    ...
    private Apple apple;
    private JPanel panelWithCounterAmdSpeedMeter;
    ...
    private void initPanelWithCounterAmdSpeedMeter(){
        panelWithCounterAmdSpeedMeter = new JPanel(new GridLayout(0, 2));
        panelWithCounterAmdSpeedMeter.setOpaque(true);
        panelWithCounterAmdSpeedMeter.setBackground(new Color(0x928AC8));
        add(panelWithCounterAmdSpeedMeter, BorderLayout.NORTH);
    }

    private void initSizeCounterLabel(){
        sizeCounterLabel = new SizeCounterLabel();
        panelWithCounterAmdSpeedMeter.add(sizeCounterLabel);
    }

    private void initSpeedMeter(){
        speedMeter = new SpeedMeter();
        panelWithCounterAmdSpeedMeter.add(speedMeter);
    }

    private void initField(){
        JPanel panelWithSquare = new JPanel(new GridLayout(FIELD_SIZE, FIELD_SIZE, 1, 1));
        panelWithSquare.setBackground(new Color(23));
        ...
        for (int i = 0; i < FIELD_SIZE; i++) {
            for (int j = 0; j < FIELD_SIZE; j++) {
                ...
                field[i][j] = square;
                panelWithSquare.add(square);
            }
        }

        add(panelWithSquare, BorderLayout.CENTER);
    }
